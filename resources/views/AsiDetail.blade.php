  ...
  <div class="border-t border-gray-200 px-4 py-5 sm:px-6">
    <dl class="grid grid-cols-1 gap-x-4 gap-y-8 sm:grid-cols-3">
      ...
      <div class="sm:col-span-3">
        <div x-data="" class="flex items-center w-full">
          ...
          <label for="toggleB" class="flex items-center cursor-pointer">
            ...
            <div class="  relative">
              <!-- input -->
              <input  onClick="askAlamat()" type="checkbox" name="courir" id="toggleB" class="sr-only">
              <!-- line -->
      ...
      <div x-show="" style="display:none;" id="formalamat" class="sm:col-span-2">
        ...
        <x-jet-input class="w-full rounded-full p-2 px-3 text-md" placeholder="Alamat Lengkap"></x-jet-input>
      </div>
      <div class="sm:col-span-1">
        <dt class="text-sm font-mediu mb-2 text-gray-500">
          Quantity
        </dt>
        <x-jet-input class="w-full rounded-full p-2 px-3 text-md" placeholder="Jumlah"></x-jet-input>
      </div>
...
<!--  
  ...
  <h5 class="card-title">{{$getAsiProductDetail->pemilik->tanggal_melahirkan}}  <span class="fs-6 text-secondary fw-bolder ms-2"> 
    {{ $getAsiProductDetail->liter_per_pack}} sks</span></h5>
    <p class="card-text">{{$getAsiProductDetail->description}}</p>
    <p class="card-text">{{$getAsiProductDetail->pemilik->name}}</p>
  
    <form action="{{ route('ProsesPesanAsi')}}" method="post">
    {{ csrf_field() }}
        
            <input type="hidden" name="asi_product_id" value="{{ $getAsiProductDetail->id }}">
